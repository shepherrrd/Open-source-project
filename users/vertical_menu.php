...
if (isset($_SESSION['log']) && $_SESSION['log'] == 1 && isset($grade_user_session) && ($grade_user_session == "3" || $grade_user_session == "2")){
	...
	$select_statut_comp = $connect->query("select active_composant from `" . $tblprefix . "composants` where nom_composant = 'vertical_menu';");
	...
	// ****************** add_menu **************************
		case "add_menu" : {
    	 ...
    	 if (!empty($menu_titre) && !empty($_POST['menu_contenu'])){
    		...
    		$select_menu_titre = $connect->query("select id_vermenu from `" . $tblprefix . "vermenu` where titre_vermenu = '$menu_titre';");
    		if (mysqli_num_rows($select_menu_titre) == 0){
    		
    			$select_max_order = $connect->query("select max(ordre_vermenu) from `" . $tblprefix . "vermenu`;");
		...
		case "edit_menu" : {
		 ...
		 if (mysqli_num_rows($select_menu) == 1) {
		  ...
		  function check_menu($type,$notre_menu,$value){
    						...
    						if (in_array($id_article,$array_art)) {
    							if ($idmenu != $id_menu){
    								while (in_array($order_new_article,$tab_order))
    									$order_new_article++;
    								$update_article = $connect->query("update `" . $tblprefix . "articles` set id_menu_ver = $id_menu, ordre_article_ver = $order_new_article where id_article = $id_article;");
    								$order_new_article++;
    							}
    						} else {
    							if ($idmenu == $id_menu)
    								$update_article = $connect->query("update `" . $tblprefix . "articles` set id_menu_ver = 0, ordre_article_ver = 1 where id_article = $id_article;");
    						}
    					}
    				}
    		...
    		$select_article = $connect->query("select id_article, id_menu_ver, titre_article, publie_article from `" . $tblprefix . "articles`;");
    		if (mysqli_num_rows($select_article) > 0){
	...
	// ****************** delete_menu **************************
		case "delete_menu" : {
			if (isset($_GET['key']) && $_GET['key'] == $key){
				$delete_menu = $connect->query("delete from `" . $tblprefix . "vermenu` where id_vermenu = $id_menu;");
			}
	...
	// ****************** orderup_menu **************************
		case "orderup_menu" : {
			if (isset($_GET['key']) && $_GET['key'] == $key){
    		$ce_menu = $connect->query ("select ordre_vermenu from `" . $tblprefix . "vermenu` where id_vermenu = $id_menu;");
				if (mysqli_num_rows($ce_menu) == 1) {
					$ordre_menu = mysqli_result($ce_menu,0,0);

    			$menu_precedent = $connect->query ("select id_vermenu, ordre_vermenu from `" . $tblprefix . "vermenu` where ordre_vermenu < $ordre_menu order by ordre_vermenu desc;");
					if (mysqli_num_rows($menu_precedent) > 0) {
						...
						$order_this_menu = $connect->query("update `" . $tblprefix . "vermenu` set ordre_vermenu = $ordremenu_precedent where id_vermenu = $id_menu;");
						$order_menu_precedent = $connect->query("update `" . $tblprefix . "vermenu` set ordre_vermenu = $ordre_menu where id_vermenu = $idmenu_precedent;");
					}
    		}
			}
			locationhref_admin("?inc=vertical_menu");
	...
	// ****************** orderdown_menu **************************
		case "orderdown_menu" : {
    		...
    		$ce_menu = $connect->query ("select ordre_vermenu from `" . $tblprefix . "vermenu` where id_vermenu = $id_menu;");
    			...
    			$menu_suivant = $connect->query ("select id_vermenu, ordre_vermenu from `" . $tblprefix . "vermenu` where ordre_vermenu > $ordre_menu order by ordre_vermenu;");
					if (mysqli_num_rows($menu_suivant) > 0) {
						...
						$order_this_menu = $connect->query("update `" . $tblprefix . "vermenu` set ordre_vermenu = $ordremenu_suivant where id_vermenu = $id_menu;");
						$order_menu_suivant = $connect->query("update `" . $tblprefix . "vermenu` set ordre_vermenu = $ordre_menu where id_vermenu = $idmenu_suivant;");
					}
    		}
			}
			locationhref_admin("?inc=vertical_menu");
	...
	// ****************** orderup_article **************************
		case "orderup_article" : {
			if (isset($_GET['key']) && $_GET['key'] == $key){
    		$cet_article = $connect->query ("select id_menu_ver, ordre_article_ver from `" . $tblprefix . "articles` where id_article = $id_article;");
				if (mysqli_num_rows($cet_article) == 1) {
					$id_menu = mysqli_result($cet_article,0,0);
					$ordre_article = mysqli_result($cet_article,0,1);

    			$article_precedent = $connect->query ("select id_article, ordre_article_ver from `" . $tblprefix . "articles` where id_menu_ver = $id_menu and ordre_article_ver < $ordre_article order by ordre_article_ver desc;");
					if (mysqli_num_rows($article_precedent) > 0) {
						...
						$order_this_article = $connect->query("update `" . $tblprefix . "articles` set ordre_article_ver = $ordrearticle_precedent where id_article = $id_article;");
						$order_article_precedent = $connect->query("update `" . $tblprefix . "articles` set ordre_article_ver = $ordre_article where id_article = $idarticle_precedent;");
					}
					locationhref_admin("?inc=vertical_menu");
    		} else locationhref_admin("?inc=vertical_menu");
			} else locationhref_admin("?inc=vertical_menu");
	...
	// ****************** orderdown_article **************************
		case "orderdown_article" : {
    		...
    		$cet_article = $connect->query ("select id_menu_ver, ordre_article_ver from `" . $tblprefix . "articles` where id_article = $id_article;");
    			...
    			$article_suivant = $connect->query ("select id_article, ordre_article_ver from `" . $tblprefix . "articles` where id_menu_ver = $id_menu and ordre_article_ver > $ordre_article order by ordre_article_ver;");
					if (mysqli_num_rows($article_suivant) > 0) {
						...
						$order_this_article = $connect->query("update `" . $tblprefix . "articles` set ordre_article_ver = $ordrearticle_suivant where id_article = $id_article;");
						$order_article_suivant = $connect->query("update `" . $tblprefix . "articles` set ordre_article_ver = $ordre_article where id_article = $idarticle_suivant;");
					}
					locationhref_admin("?inc=vertical_menu");
    		} else locationhref_admin("?inc=vertical_menu");
			} else locationhref_admin("?inc=vertical_menu");
	...
	// ****************** activer_menu **************************
		case "activer_menu" : {
			if (isset($_GET['key']) && $_GET['key'] == $key){
    		$activer_menu = $connect->query("update `" . $tblprefix . "vermenu` set active_vermenu = '1' where id_vermenu = $id_menu;");
			}
			locationhref_admin("?inc=vertical_menu");
		} break;
	...
	// ****************** desactiver_menu **************************
		case "desactiver_menu" : {
			if (isset($_GET['key']) && $_GET['key'] == $key){
    		$desactiver_menu = $connect->query("update `" . $tblprefix . "vermenu` set active_vermenu = '0' where id_vermenu = $id_menu;");
			}
			locationhref_admin("?inc=vertical_menu");
		} break;
   	...
   	// ****************** liste_menu **************************
		default : {
			...
			$select_menu = $connect->query("select * from `" . $tblprefix . "vermenu` order by ordre_vermenu;");
  		...
  		if ($nbr_trouve > 0){
				...
				$select_menu_limit = $connect->query("select * from `" . $tblprefix . "vermenu` order by ordre_vermenu limit $limit, $nbr_resultats;");
				...
				if ($type_menu == "article"){
		 			$select_articles = $connect->query("select id_article, titre_article, contenu_article, ordre_article_ver from `" . $tblprefix . "articles` where id_menu_ver = $id_menu order by ordre_article_ver;");
		 			if (mysqli_num_rows($select_articles) > 0){
						...
						while($article = mysqli_fetch_row($select_articles)){
							...
							echo "\n<td nowrap=\"nowrap\">";
							$article_precedent = $connect->query ("select id_article from `" . $tblprefix . "articles` where ordre_article_ver < $ordre_article and id_menu_ver = $id_menu order by ordre_article_ver desc;");
							if (mysqli_num_rows($article_precedent) > 0)
							...
							echo "<b> ".$j_ordre." </b>";
							$j_ordre++;
							$article_suivant = $connect->query ("select id_article from `" . $tblprefix . "articles` where ordre_article_ver > $ordre_article and id_menu_ver = $id_menu order by ordre_article_ver;");
							if (mysqli_num_rows($article_suivant) > 0)
					...
					$menu_precedent = $connect->query ("select id_vermenu from `" . $tblprefix . "vermenu` where ordre_vermenu < $ordre_menu order by ordre_vermenu desc;");
					if (mysqli_num_rows($menu_precedent) > 0)
					...
					echo "<b> ".$i_ordre." </b>";
					$i_ordre++;
					$menu_suivant = $connect->query ("select id_vermenu from `" . $tblprefix . "vermenu` where ordre_vermenu > $ordre_menu order by ordre_vermenu;");
					if (mysqli_num_rows($menu_suivant) > 0)
