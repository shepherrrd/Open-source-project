...
if (isset($_SESSION['log']) && $_SESSION['log'] == 1){
	...
	// ****************** open_msg **************************
		case "open_msg" : {
			...
			if ($grade_user_session == "3" || $grade_user_session == "2")
				$select_un_msg = mysql_query("select * from `" . $tblprefix . "messages` where id_message = $id_msg and (id_destinataires like '%-$id_user_session-%' or (id_destinataires = '*' and id_destinataires_app = '*') or id_emetteur = $id_user_session);");
			else
				$select_un_msg = mysql_query("select * from `" . $tblprefix . "messages` where id_message = $id_msg and (id_destinataires like '%-$id_user_session-%' or id_emetteur = $id_user_session);");
			...
			if (mysql_num_rows($select_un_msg) == 1) {
				...
				$mail_msg = html_ent(mysql_result($select_un_msg,0,4));
				...
				$lu_message = mysql_result($select_un_msg,0,9);
				...
				if($id_emetteur == 0 && $id_emetteur_app == 0){
					$emetteur = html_ent(mysql_result($select_un_msg,0,3));
				...
				if (strpos($lu_message,"-".$id_user_session."-") === false){
					...
					$update_lu_message = mysql_query("update `" . $tblprefix . "messages` set lu_message = '$lu_message' where id_message = $id_msg;");
				}
    		...
    		echo "</ul>";
				...
				if ($id_emetteur == $id_user_session){
						$msg_precedent = mysql_query ("select id_message from `" . $tblprefix . "messages` where date_message > $ladate and id_emetteur = $id_user_session order by date_message;");
						$msg_suivant   = mysql_query ("select id_message from `" . $tblprefix . "messages` where date_message < $ladate and id_emetteur = $id_user_session order by date_message desc;");
				}
				else{
					if ($grade_user_session == "3" || $grade_user_session == "2"){
						$msg_precedent = mysql_query ("select id_message from `" . $tblprefix . "messages` where date_message > $ladate and (id_destinataires like '%-$id_user_session-%' or (id_destinataires = '*' and id_destinataires_app = '*')) order by date_message;");
						$msg_suivant   = mysql_query ("select id_message from `" . $tblprefix . "messages` where date_message < $ladate and (id_destinataires like '%-$id_user_session-%' or (id_destinataires = '*' and id_destinataires_app = '*')) order by date_message desc;");
					}
					else {
						$msg_precedent = mysql_query ("select id_message from `" . $tblprefix . "messages` where date_message > $ladate and id_destinataires like '%-$id_user_session-%' order by date_message;");
						$msg_suivant   = mysql_query ("select id_message from `" . $tblprefix . "messages` where date_message < $ladate and id_destinataires like '%-$id_user_session-%' order by date_message desc;");
					}
				}
	...
	// ****************** delete_msg **************************
		case "delete_msg" : {
			if (isset($_GET['key']) && $_GET['key'] == $key){
				$select_dest_msg = mysql_query("select id_destinataires, id_destinataires_app from `" . $tblprefix . "messages` where id_message = $id_msg;");
				if (mysql_num_rows($select_dest_msg) == 1) {
					$id_destinataires = mysql_result($select_dest_msg,0,0);
					$id_destinataires_app = mysql_result($select_dest_msg,0,1);
					if ($id_destinataires == "*" && $id_destinataires_app == "*" && ($grade_user_session == "3" || $grade_user_session == "2"))
						$delete_msg = mysql_query("delete from `" . $tblprefix . "messages` where id_message = $id_msg;");
						...
						$update_dest_message = mysql_query("update `" . $tblprefix . "messages` set id_destinataires = '$id_destinataires2' where id_message = $id_msg;");
					}
				}
			}
	...
	// ****************** delete_outbox_msg **************************
		case "delete_outbox_msg" : {
			if (isset($_GET['key']) && $_GET['key'] == $key){
				$select_outbox_msg = mysql_query("select id_emetteur from `" . $tblprefix . "messages` where id_message = $id_msg;");
	...
	// ****************** new_msg **************************
		case "new_msg" : {
    	 ...
    	 if (!isset($_SESSION['random_key']) || $_SESSION['random_key'] != $_POST['random']){
				  ...
				  if ($need_classe == 1){
				  	if (isset($_POST['classes_app']) && !empty($_POST['classes_app'])){
				  		foreach ($_POST['classes_app'] as $elem_classe){
				  			$select_app_classe = mysql_query("select id_apprenant from `" . $tblprefix . "apprenants` where id_classe = $elem_classe;");
    		...
    		$select_users = mysql_query("select id_user, identifiant_user, grade_user from `" . $tblprefix . "users` where id_user != $id_user_session order by grade_user desc;");
    		$select_apprenants = mysql_query("select id_apprenant, id_classe, identifiant_apprenant from `" . $tblprefix . "apprenants` order by id_classe desc;");
	...
	// ****************** reply **************************
		case "reply" : {
			...
			$select_this_msg = mysql_query("select * from `" . $tblprefix . "messages` where id_message = $id_msg and id_destinataires like '%-$id_user_session-%';");
	...
	// ****************** forward **************************
		case "forward" : {
		 ...
		 $select_this_msg = mysql_query("select * from `" . $tblprefix . "messages` where id_message = $id_msg and (id_destinataires like '%-$id_user_session-%' or id_emetteur = $id_user_session);");
     if (mysql_num_rows($select_this_msg) == 1) {
	...
	$select_msg = mysql_query("select * from `" . $tblprefix . "messages` where id_emetteur = $id_user_session and deleted_from_outbox = '0' order by date_message desc;"); 
  ...
  if ($nbr_trouve > 0){
		...
		if ($page <= $page_max && $page > 1 && $page_max > 1)
			...
			$select_msg_limit = mysql_query("select * from `" . $tblprefix . "messages` where id_emetteur = $id_user_session and deleted_from_outbox = '0' order by date_message desc limit $limit, $nbr_resultats;"); 
				...
				while($msg = mysql_fetch_row($select_msg_limit)){
				  ...
				  $select_users = mysql_query("select id_user, identifiant_user from `" . $tblprefix . "users` where id_user != $id_user_session;");
    			if (mysql_num_rows($select_users) > 0){
    				while($user = mysql_fetch_row($select_users)){
    					$id_user = $user[0];
    			...
    			$select_apps = mysql_query("select id_apprenant, identifiant_apprenant from `" . $tblprefix . "apprenants`;");
  ...
  // ****************** liste_msg **************************	
	...
	$select_msg = mysql_query("select * from `" . $tblprefix . "messages` where id_destinataires like '%-$id_user_session-%' order by date_message desc;"); 
	$nbr_trouve = mysql_num_rows($select_msg);
  if ($nbr_trouve > 0){
		$page_max = ceil($nbr_trouve / $nbr_resultats);
		if ($page <= $page_max && $page > 1 && $page_max > 1)
		...
		$select_msg_limit = mysql_query("select * from `" . $tblprefix . "messages` where id_destinataires like '%-$id_user_session-%' order by date_message desc limit $limit, $nbr_resultats;"); 
		...
		// visitor messages
			...
			if ($grade_user_session == "3" || $grade_user_session == "2"){
			 ...
			 $select_msg = mysql_query("select * from `" . $tblprefix . "messages` where id_destinataires = '*' and id_destinataires_app = '*' order by date_message desc;"); 
			 $nbr_trouve = mysql_num_rows($select_msg);
  		 if ($nbr_trouve > 0){
				$page_max = ceil($nbr_trouve / $nbr_resultats);
			 ...
			 $select_msg_limit = mysql_query("select * from `" . $tblprefix . "messages` where id_destinataires = '*' and id_destinataires_app = '*' order by date_message desc limit $limit, $nbr_resultats;"); 
				...
				while($msg = mysql_fetch_row($select_msg_limit)){
					...
					echo "\n<td class=\"affichage_table\"><b>";
				  $select_users = mysql_query("select id_user, identifiant_user from `" . $tblprefix . "users` where id_user != $id_user_session and (grade_user = '2' or grade_user = '3');");
    			if (mysql_num_rows($select_users) > 0){
    				while($user = mysql_fetch_row($select_users)){
    					$id_user = $user[0];
    					$identifiant_user = html_ent($user[1]);
