...
if (isset($_SESSION['log']) && $_SESSION['log'] == 1 && isset($grade_user_session) && $grade_user_session == "0"){
	...
	if (mysqli_num_rows($select_statut_comp) == 1) {
		...
		if ($statut_comp == 0)
		 echo "<h3><img src=\"../images/icones/warning.png\" /><font color=\"red\">".component_disabled."</font></h3>";
	}
    	 ...
    	 if (!empty($tuto_titre)){
    		...
    		$select_tuto_titre = $connect->query("select id_tutoriel from `" . $tblprefix . "tutoriels` where titre_tutoriel = '$tuto_titre';");
 					if (mysqli_num_rows($select_tuto_titre) == 0) {
 						...
 						$inserttuto = "INSERT INTO `" . $tblprefix . "tutoriels` VALUES (NULL,$id_user_session,'$tuto_titre','','','','by','','0',$ordre_tuto,$time_insert_tuto,$time_insert_tuto,0,'*',0,0);";
	          $connect->query($inserttuto);
	          if ($this_tuto_insert = $connect->insert_id)
	          	$link = "?inc=tutorials&do=update_tuto&id_tuto=".$this_tuto_insert;
    			...
    			if ($tutoriel[1] == $id_user_session) {
    			 if ($tutoriel[8] != 2) {
    				if (!empty($_POST['send'])){
    					...
    					echo "<br /><p><u><b>" .notes_tuto. "</b></u><br /><br /><textarea name=\"tuto_notes\" cols=\"100\" rows=\"10\">".$notes_tuto."</textarea></p>";
    ...
    case "delete_tuto" : {
    	if (isset($_GET['key']) && $_GET['key'] == $key){
    		$select_user = $connect->query("select id_user from `" . $tblprefix . "tutoriels` where id_tutoriel = $id_tuto;");
    ...
    case "publier_tuto" : {
    	if (isset($_GET['key']) && $_GET['key'] == $key){
    		$publier_tuto = $connect->query("update `" . $tblprefix . "tutoriels` set publie_tutoriel = '1' where id_tutoriel = $id_tuto and id_user = $id_user_session;");
    	}
    	locationhref_admin("?inc=tutorials");
    } break;
    ...
    case "depublier_tuto" : {
    	if (isset($_GET['key']) && $_GET['key'] == $key){
    		$depublier_tuto = $connect->query("update `" . $tblprefix . "tutoriels` set publie_tutoriel = '0' where id_tutoriel = $id_tuto and id_user = $id_user_session;");
    	}
    	locationhref_admin("?inc=tutorials");
    } break;
    ...
    case "open_tuto" : {
		 ...
		 goback_lien("?inc=tutorials");
	
     $select_tuto = $connect->query("select titre_tutoriel, publie_tutoriel, notes_tutoriel from `" . $tblprefix . "tutoriels` where id_tutoriel = $id_tuto and id_user = $id_user_session;");
     if (mysqli_num_rows($select_tuto) == 1) {
    	$titre_tuto = mysqli_result($select_tuto,0,0);
    	$publie_tuto = mysqli_result($select_tuto,0,1);
    	$notes_tuto = mysqli_result($select_tuto,0,2);
  ...
  if ($nbr_trouve > 0){
    ...
    case "create_partie" : {
     ...
     $select_publie_tuto = $connect->query("select id_user, publie_tutoriel from `" . $tblprefix . "tutoriels` where id_tutoriel = $id_tuto;");
     $id_user = mysqli_result($select_publie_tuto,0,0);
     $publie_tuto = mysqli_result($select_publie_tuto,0,1);
    ...
    // ****************** update_partie ************************
    case "update_partie" : {
    	$select_tuto = $connect->query("select `" . $tblprefix . "tutoriels`.id_tutoriel, id_user, publie_tutoriel from `" . $tblprefix . "tutoriels`, `" . $tblprefix . "parties` where `" . $tblprefix . "parties`.id_tutoriel = `" . $tblprefix . "tutoriels`.id_tutoriel and id_partie = $id_part;");
    	$id_tuto = mysqli_result($select_tuto,0,0);
    	$id_user = mysqli_result($select_tuto,0,1);
    	$publie_tuto = mysqli_result($select_tuto,0,2);
    	if ($publie_tuto != 2 && $id_user == $id_user_session) {
    ...
    case "delete_partie" : {
			if (isset($_GET['key']) && $_GET['key'] == $key){
    		$select_user_idtuto = $connect->query("select id_user, `" . $tblprefix . "tutoriels`.id_tutoriel from `" . $tblprefix . "tutoriels`, `" . $tblprefix . "parties` where `" . $tblprefix . "tutoriels`.id_tutoriel = `" . $tblprefix . "parties`.id_tutoriel and id_partie = $id_part;");
    		if (mysqli_num_rows($select_user_idtuto) == 1 && mysqli_result($select_user_idtuto,0,0) == $id_user_session){
    			$id_tutoriel = mysqli_result($select_user_idtuto,0,1);
    	...
    	if (isset($_GET['key']) && $_GET['key'] == $key){
    		$cette_partie = $connect->query ("select id_tutoriel, ordre_partie from `" . $tblprefix . "parties` where id_partie = $id_part;");
				if (mysqli_num_rows($cette_partie) == 1) {
					...
					$select_user = $connect->query("select id_user from `" . $tblprefix . "tutoriels` where id_tutoriel = $id_tutoriel;");
					if (mysqli_num_rows($select_user) == 1 && mysqli_result($select_user,0) == $id_user_session){
    ...
    case "open_partie" : {
     ...
     $select_tuto = $connect->query("select `" . $tblprefix . "tutoriels`.id_tutoriel, titre_tutoriel, titre_partie, publie_tutoriel from `" . $tblprefix . "tutoriels`, `" . $tblprefix . "parties` where `" . $tblprefix . "tutoriels`.id_tutoriel = `" . $tblprefix . "parties`.id_tutoriel and id_partie = $id_part and id_user = $id_user_session;");
     if (mysqli_num_rows($select_tuto) == 1) {
     	$id_tuto = mysqli_result($select_tuto,0,0);
    	...
    	$publie_tuto = mysqli_result($select_tuto,0,3);
     ...
     if (!empty($_POST['send'])){
    		...
    		locationhref_admin("?inc=tutorials&do=open_partie&id_part=".$id_part);
     }
     else {
		
			goback_lien("?inc=tutorials&do=open_tuto&id_tuto=".$id_tuto);
  ...
  if ($nbr_trouve > 0){
    ...
    case "create_chapitre" : {
    	$select_publie_user_tuto = $connect->query("select id_user, `" . $tblprefix . "tutoriels`.id_tutoriel, publie_tutoriel from `" . $tblprefix . "tutoriels`, `" . $tblprefix . "parties` where `" . $tblprefix . "tutoriels`.id_tutoriel = `" . $tblprefix . "parties`.id_tutoriel and id_partie = $id_part;");
     	$id_user = mysqli_result($select_publie_user_tuto,0,0);
     	$id_tuto = mysqli_result($select_publie_user_tuto,0,1);
     	$publie_tuto = mysqli_result($select_publie_user_tuto,0,2);
     	if ($publie_tuto != 2 && $id_user == $id_user_session) {
    ...
    case "update_chapitre" : {
    	...
    	$select_tuto_partie = $connect->query("select `" . $tblprefix . "chapitres`.id_partie, id_user, publie_tutoriel, `" . $tblprefix . "tutoriels`.id_tutoriel from `" . $tblprefix . "tutoriels`, `" . $tblprefix . "parties`, `" . $tblprefix . "chapitres` where `" . $tblprefix . "parties`.id_tutoriel = `" . $tblprefix . "tutoriels`.id_tutoriel and `" . $tblprefix . "parties`.id_partie = `" . $tblprefix . "chapitres`.id_partie and id_chapitre = $id_chap;");
    	
    	$id_partie = mysqli_result($select_tuto_partie,0,0);
    	$id_user = mysqli_result($select_tuto_partie,0,1);
    	$publie_tuto = mysqli_result($select_tuto_partie,0,2);
    	$id_tuto = mysqli_result($select_tuto_partie,0,3);
    	if ($publie_tuto != 2 && $id_user == $id_user_session) {
    ...
    case "delete_chapitre" : {
    	if (isset($_GET['key']) && $_GET['key'] == $key){
    		$select_user_idpartie = $connect->query("select id_user, `" . $tblprefix . "parties`.id_partie from `" . $tblprefix . "tutoriels`, `" . $tblprefix . "parties`, `" . $tblprefix . "chapitres` where `" . $tblprefix . "tutoriels`.id_tutoriel = `" . $tblprefix . "parties`.id_tutoriel and `" . $tblprefix . "parties`.id_partie = `" . $tblprefix . "chapitres`.id_partie and id_chapitre = $id_chap;");
    		if (mysqli_num_rows($select_user_idpartie) == 1 && mysqli_result($select_user_idpartie,0,0) == $id_user_session){
    			$id_partie = mysqli_result($select_user_idpartie,0,1);
    ...
    // ****************** open_chapitre ************************
    case "open_chapitre" : {
     $select_tuto = $connect->query("select `" . $tblprefix . "parties`.id_partie, `" . $tblprefix . "tutoriels`.id_tutoriel, titre_tutoriel, titre_partie, titre_chapitre, publie_tutoriel from `" . $tblprefix . "tutoriels`, `" . $tblprefix . "parties`, `" . $tblprefix . "chapitres` where `" . $tblprefix . "tutoriels`.id_tutoriel = `" . $tblprefix . "parties`.id_tutoriel and `" . $tblprefix . "parties`.id_partie = `" . $tblprefix . "chapitres`.id_partie and id_chapitre = $id_chap and id_user = $id_user_session;");
     if (mysqli_num_rows($select_tuto) == 1) {
    	...
    	$publie_tuto = mysqli_result($select_tuto,0,5);

			goback_lien("?inc=tutorials&do=open_partie&id_part=".$id_part);
  ...
  if ($nbr_trouve > 0){
		...
		if ($page_max > 1){
  		...
  		echo "<br /><table border=\"0\" align=\"center\"><tr>";
			if ($page_precedente >= 1)
				echo "<td><a href=\"?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."&l=".$page_precedente."&t=".$page2."&d=".$page3."#blocs\"><img border=\"0\" src=\"../images/others/precedent.png\" width=\"32\" height=\"32\" /></a></td><td><a href=\"?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."&l=".$page_precedente."&t=".$page2."&d=".$page3."#blocs\"><b>".page_precedente."</b></a></td>";
			echo "<td>";
			for($i=1;$i<=$page_max;$i++){
				if ($i != $page) echo "<a href=\"?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."&l=".$i."&t=".$page2."&d=".$page3."#blocs\">";
				echo "<b>".$i."</b>";
				if ($i != $page) echo "</a>";
				echo "&nbsp; ";
			}
			echo "</td>";
			if ($page_suivante <= $page_max)
				echo "<td><a href=\"?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."&l=".$page_suivante."&t=".$page2."&d=".$page3."#blocs\"><b>".page_suivante."</b></a></td><td><a href=\"?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."&l=".$page_suivante."&t=".$page2."&d=".$page3."#blocs\"><img border=\"0\" src=\"../images/others/suivant.png\" width=\"32\" height=\"32\" /></a></td>";
			echo "</tr></table>";
		}
		...
		if ($page_max > 1){
  		...
  		echo "<br /><table border=\"0\" align=\"center\"><tr>";
			if ($page_precedente >= 1)
				echo "<td><a href=\"?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."&t=".$page_precedente."&l=".$page."&d=".$page3."#qcm\"><img border=\"0\" src=\"../images/others/precedent.png\" width=\"32\" height=\"32\" /></a></td><td><a href=\"?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."&t=".$page_precedente."&l=".$page."&d=".$page3."#qcm\"><b>".page_precedente."</b></a></td>";
			echo "<td>";
			for($i=1;$i<=$page_max;$i++){
				if ($i != $page2) echo "<a href=\"?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."&t=".$i."&l=".$page."&d=".$page3."#qcm\">";
				echo "<b>".$i."</b>";
				if ($i != $page2) echo "</a>";
				echo "&nbsp; ";
			}
			echo "</td>";
			if ($page_suivante <= $page_max)
				echo "<td><a href=\"?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."&t=".$page_suivante."&l=".$page."&d=".$page3."#qcm\"><b>".page_suivante."</b></a></td><td><a href=\"?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."&t=".$page_suivante."&l=".$page."&d=".$page3."#qcm\"><img border=\"0\" src=\"../images/others/suivant.png\" width=\"32\" height=\"32\" /></a></td>";
			echo "</tr></table>";
		}
    	...
    	// ***************** Homework ********************
			...
			if ($publie_tuto == 2)
    		echo "<table border=\"0\"><tr><td><img border=\"0\" src=\"../images/others/add2.png\" /></a></td><td><b>".creer_devoir."</b></td></tr></table><br />";
 			else
 				echo "<table border=\"0\"><tr><td><a href=\"?inc=tutorials&do=create_devoir&id_chap=".$id_chap."\"><img border=\"0\" src=\"../images/others/add.png\" /></a></td><td><a href=\"?inc=tutorials&do=create_devoir&id_chap=".$id_chap."\"><b>".creer_devoir."</b></a></td></tr></table><br />";
    	...
    	$select_my_devoir_limit = $connect->query("select * from `" . $tblprefix . "devoirs` where id_chapitre = $id_chap order by ordre_devoir limit $limit, $nbr_resultats;");
    		...
    		echo "<table width=\"100%\" align=\"center\" style=\"border: 1px solid #000000;\"><tr bgcolor=\"#f1d3bd\">\n";
				...
				while($devoir = mysqli_fetch_row($select_my_devoir_limit)){
					...
					echo "<tr>\n";
					echo "\n<td class=\"affichage_table\"><a href=\"?inc=tutorials&do=open_devoir&id_devoir=".$devoir[0]."\" title=\"".ouvrir_devoir."\"><font color=\"".$color."\"><u><b>".$titre_devoir."</b></u></font></a></td>";
					echo "\n<td class=\"affichage_table\"><b>".$acces_devoir."</b></td>";
					...
					if ($publie_tuto == 2)
						...
						echo "\n<td class=\"affichage_table\"><a href=\"?inc=tutorials&do=update_devoir&id_devoir=".$devoir[0]."\" title=\"".editer."\"><img border=\"0\" src=\"../images/others/edit.png\" width=\"32\" height=\"32\" /></a></td>";
					
					echo "\n<td class=\"affichage_table\"><a href=\"#\" onClick=\"confirmer('?inc=tutorials&do=delete_devoir&id_devoir=".$devoir[0]."&key=".$key."','".confirm_supprimer_devoir."\\n".confirm_supprimer_devoir2."\\n".confirm_supprimer_devoir3."')\" title=\"".supprimer."\"><img border=\"0\" src=\"../images/others/delete.png\" width=\"32\" height=\"32\" /></a></td>";
					...
					if (mysqli_num_rows($devoir_precedent) > 0)
						echo "<a href=\"?inc=tutorials&do=orderup_devoir&id_devoir=".$devoir[0]."&key=".$key."\" title=\"".deplacer_haut."\"><img border=\"0\" src=\"../images/others/up.png\" width=\"15\" height=\"15\" /></a>";
					else
						echo "<img border=\"0\" src=\"../images/others/up2.png\" width=\"15\" height=\"15\" />";
					echo "<b> ".$d_ordre." </b>";
					$d_ordre++;
					...
					if (mysqli_num_rows($devoir_suivant) > 0)
						echo "<a href=\"?inc=tutorials&do=orderdown_devoir&id_devoir=".$devoir[0]."&key=".$key."\" title=\"".deplacer_bas."\"><img border=\"0\" src=\"../images/others/down.png\" width=\"15\" height=\"15\" /></a>";
					else echo "<img border=\"0\" src=\"../images/others/down2.png\" width=\"15\" height=\"15\" />";
					echo "</td>";
					
					if ($devoir[7] == 1)
						echo "\n<td class=\"affichage_table\"><a href=\"?inc=tutorials&do=depublier_devoir&id_devoir=".$devoir[0]."&key=".$key."\" title=\"".depublier_element."\"><b>".depublier."</b></a></td>";
					else
						echo "\n<td class=\"affichage_table\"><a href=\"?inc=tutorials&do=publier_devoir&id_devoir=".$devoir[0]."&key=".$key."\" title=\"".publier_element."\"><b>".publier."</b></a></td>";
		...
		if ($page_max > 1){
  		...
  		echo "<br /><table border=\"0\" align=\"center\"><tr>";
			if ($page_precedente >= 1)
				echo "<td><a href=\"?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."&d=".$page_precedente."&l=".$page."&t=".$page2."#devoir\"><img border=\"0\" src=\"../images/others/precedent.png\" width=\"32\" height=\"32\" /></a></td><td><a href=\"?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."&d=".$page_precedente."&l=".$page."&t=".$page2."#devoir\"><b>".page_precedente."</b></a></td>";
			echo "<td>";
			for($i=1;$i<=$page_max;$i++){
				if ($i != $page3) echo "<a href=\"?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."&d=".$i."&l=".$page."&t=".$page2."#devoir\">";
				echo "<b>".$i."</b>";
				if ($i != $page3) echo "</a>";
				echo "&nbsp; ";
			}
			echo "</td>";
			if ($page_suivante <= $page_max)
				echo "<td><a href=\"?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."&d=".$page_suivante."&l=".$page."&t=".$page2."#devoir\"><b>".page_suivante."</b></a></td><td><a href=\"?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."&d=".$page_suivante."&l=".$page."&t=".$page2."#devoir\"><img border=\"0\" src=\"../images/others/suivant.png\" width=\"32\" height=\"32\" /></a></td>";
			echo "</tr></table>";
		}
    ...
    // ****************** create_bloc **************************
    case "create_bloc" : {
    	$select_publie_user_tuto = $connect->query("select id_user, publie_tutoriel, `" . $tblprefix . "tutoriels`.id_tutoriel from `" . $tblprefix . "tutoriels`, `" . $tblprefix . "parties`, `" . $tblprefix . "chapitres` where `" . $tblprefix . "tutoriels`.id_tutoriel = `" . $tblprefix . "parties`.id_tutoriel and `" . $tblprefix . "parties`.id_partie = `" . $tblprefix . "chapitres`.id_partie and id_chapitre = $id_chap;");
     	$id_user = mysqli_result($select_publie_user_tuto,0,0);
     	$publie_tuto = mysqli_result($select_publie_user_tuto,0,1);
     	$id_tuto = mysqli_result($select_publie_user_tuto,0,2);
     	if ($publie_tuto != 2 && $id_user == $id_user_session) {
    ...
    case "delete_bloc" : {
    	if (isset($_GET['key']) && $_GET['key'] == $key){
    		$select_user_idchap = $connect->query("select id_user, `" . $tblprefix . "blocs`.id_chapitre from `" . $tblprefix . "tutoriels`, `" . $tblprefix . "parties`, `" . $tblprefix . "chapitres`, `" . $tblprefix . "blocs` where `" . $tblprefix . "tutoriels`.id_tutoriel = `" . $tblprefix . "parties`.id_tutoriel and `" . $tblprefix . "parties`.id_partie = `" . $tblprefix . "chapitres`.id_partie and `" . $tblprefix . "chapitres`.id_chapitre = `" . $tblprefix . "blocs`.id_chapitre and id_bloc = $id_bloc;");
    		if (mysqli_num_rows($select_user_idchap) == 1 && mysqli_result($select_user_idchap,0,0) == $id_user_session){
    			$id_chapitre = mysqli_result($select_user_idchap,0,1);
    ...
    case "orderup_bloc" : {
    	if (isset($_GET['key']) && $_GET['key'] == $key){
    		$ce_bloc = $connect->query ("select id_tutoriel, `" . $tblprefix . "blocs`.id_chapitre, ordre_bloc from `" . $tblprefix . "parties`, `" . $tblprefix . "chapitres`, `" . $tblprefix . "blocs` where `" . $tblprefix . "chapitres`.id_partie = `" . $tblprefix . "parties`.id_partie and `" . $tblprefix . "blocs`.id_chapitre = `" . $tblprefix . "chapitres`.id_chapitre and id_bloc = $id_bloc;");
				if (mysqli_num_rows($ce_bloc) == 1) {
					$id_tutoriel = mysqli_result($ce_bloc,0,0);
					$id_chapitre = mysqli_result($ce_bloc,0,1);
					$ordre_bloc = mysqli_result($ce_bloc,0,2);
    ...
    // ****************** update_qcm **************************
    case "update_qcm" : {
    	$select_tuto_chapitre = $connect->query("select `" . $tblprefix . "qcm`.id_chapitre, id_user, publie_tutoriel, `" . $tblprefix . "tutoriels`.id_tutoriel from `" . $tblprefix . "tutoriels`, `" . $tblprefix . "parties`, `" . $tblprefix . "chapitres`, `" . $tblprefix . "qcm` where `" . $tblprefix . "parties`.id_tutoriel = `" . $tblprefix . "tutoriels`.id_tutoriel and `" . $tblprefix . "parties`.id_partie = `" . $tblprefix . "chapitres`.id_partie and `" . $tblprefix . "chapitres`.id_chapitre = `" . $tblprefix . "qcm`.id_chapitre and id_qcm = $id_qcm;");
    ...
    case "delete_qcm" : {
    	if (isset($_GET['key']) && $_GET['key'] == $key){
    		$select_user_idchap = $connect->query("select id_user, `" . $tblprefix . "qcm`.id_chapitre from `" . $tblprefix . "tutoriels`, `" . $tblprefix . "parties`, `" . $tblprefix . "chapitres`, `" . $tblprefix . "qcm` where `" . $tblprefix . "tutoriels`.id_tutoriel = `" . $tblprefix . "parties`.id_tutoriel and `" . $tblprefix . "parties`.id_partie = `" . $tblprefix . "chapitres`.id_partie and `" . $tblprefix . "chapitres`.id_chapitre = `" . $tblprefix . "qcm`.id_chapitre and id_qcm = $id_qcm;");
    		if (mysqli_num_rows($select_user_idchap) == 1 && mysqli_result($select_user_idchap,0,0) == $id_user_session){
    			$id_chapitre = mysqli_result($select_user_idchap,0,1);
    ...
    case "orderup_qcm" : {
    	if (isset($_GET['key']) && $_GET['key'] == $key){
    		$ce_qcm = $connect->query ("select id_tutoriel, `" . $tblprefix . "qcm`.id_chapitre, ordre_qcm from `" . $tblprefix . "parties`, `" . $tblprefix . "chapitres`, `" . $tblprefix . "qcm` where `" . $tblprefix . "chapitres`.id_partie = `" . $tblprefix . "parties`.id_partie and `" . $tblprefix . "qcm`.id_chapitre = `" . $tblprefix . "chapitres`.id_chapitre and id_qcm = $id_qcm;");
				if (mysqli_num_rows($ce_qcm) == 1) {
					$id_tutoriel = mysqli_result($ce_qcm,0,0);
					$id_chapitre = mysqli_result($ce_qcm,0,1);
					$ordre_qcm = mysqli_result($ce_qcm,0,2);
     		 ...
     		 if (!isset($_SESSION['random_key']) || $_SESSION['random_key'] != $_POST['random']){
     			...
     			if (!empty($titre_devoir) && !empty($contenu_devoir) && !empty($date_publication) && !empty($duree_publication)){
	          	...
	          	$date_modification_tuto_chap = time();
	         		...
	         		$link = "?inc=tutorials&do=open_chapitre&id_chap=".$id_chap."#devoir";
    ...
    case "update_devoir" : {
     ...
     $select_tuto_chapitre = $connect->query("select `" . $tblprefix . "devoirs`.id_chapitre, id_user, publie_tutoriel, `" . $tblprefix . "tutoriels`.id_tutoriel from `" . $tblprefix . "tutoriels`, `" . $tblprefix . "parties`, `" . $tblprefix . "chapitres`, `" . $tblprefix . "devoirs` where `" . $tblprefix . "parties`.id_tutoriel = `" . $tblprefix . "tutoriels`.id_tutoriel and `" . $tblprefix . "parties`.id_partie = `" . $tblprefix . "chapitres`.id_partie and `" . $tblprefix . "chapitres`.id_chapitre = `" . $tblprefix . "devoirs`.id_chapitre and id_devoir = $id_devoir;");
     		 ...
     		 if (!isset($_SESSION['random_key']) || $_SESSION['random_key'] != $_POST['random']){
     			...
     			if (!empty($titre_devoir) && !empty($contenu_devoir) && !empty($date_publication) && !empty($duree_publication)){
	          	...
	          	$date_modification_tuto_chap = time();
	         		...
	         		$link = "?inc=tutorials&do=open_chapitre&id_chap=".$id_chapitre."#devoir";
    ...
    case "delete_devoir" : {
    	if (isset($_GET['key']) && $_GET['key'] == $key){
    		$select_user_idchap = $connect->query("select id_user, `" . $tblprefix . "devoirs`.id_chapitre from `" . $tblprefix . "tutoriels`, `" . $tblprefix . "parties`, `" . $tblprefix . "chapitres`, `" . $tblprefix . "devoirs` where `" . $tblprefix . "tutoriels`.id_tutoriel = `" . $tblprefix . "parties`.id_tutoriel and `" . $tblprefix . "parties`.id_partie = `" . $tblprefix . "chapitres`.id_partie and `" . $tblprefix . "chapitres`.id_chapitre = `" . $tblprefix . "devoirs`.id_chapitre and id_devoir = $id_devoir;");
    		if (mysqli_num_rows($select_user_idchap) == 1 && mysqli_result($select_user_idchap,0,0) == $id_user_session){
    			$id_chapitre = mysqli_result($select_user_idchap,0,1);
    			...
    			$select_titre_devoir = $connect->query("select titre_devoir from `" . $tblprefix . "devoirs` where id_devoir = $id_devoir;");
					...
					locationhref_admin("?inc=tutorials&do=open_chapitre&id_chap=".$id_chapitre."#devoir");
    		} else locationhref_admin("?inc=tutorials");
    	} else locationhref_admin("?inc=tutorials");
    } break;
    ...
    case "orderup_devoir" : {
    	if (isset($_GET['key']) && $_GET['key'] == $key){
    		$ce_devoir = $connect->query ("select id_tutoriel, `" . $tblprefix . "devoirs`.id_chapitre, ordre_devoir from `" . $tblprefix . "parties`, `" . $tblprefix . "chapitres`, `" . $tblprefix . "devoirs` where `" . $tblprefix . "chapitres`.id_partie = `" . $tblprefix . "parties`.id_partie and `" . $tblprefix . "devoirs`.id_chapitre = `" . $tblprefix . "chapitres`.id_chapitre and id_devoir = $id_devoir;");
				if (mysqli_num_rows($ce_devoir) == 1) {
					$id_tutoriel = mysqli_result($ce_devoir,0,0);
					$id_chapitre = mysqli_result($ce_devoir,0,1);
					$ordre_devoir = mysqli_result($ce_devoir,0,2);
  ...
  case "delete_devoir_rendu" : {
  	if (isset($_GET['key']) && $_GET['key'] == $key){
  		if (isset($_GET['id_devoir_rendu']) && ctype_digit($_GET['id_devoir_rendu']))
    		...
    		$select_user_devoir = $connect->query("select id_user from `" . $tblprefix . "tutoriels`, `" . $tblprefix . "parties`, `" . $tblprefix . "chapitres`, `" . $tblprefix . "devoirs` where `" . $tblprefix . "tutoriels`.id_tutoriel = `" . $tblprefix . "parties`.id_tutoriel and `" . $tblprefix . "parties`.id_partie = `" . $tblprefix . "chapitres`.id_partie and `" . $tblprefix . "chapitres`.id_chapitre = `" . $tblprefix . "devoirs`.id_chapitre and id_devoir = $id_devoir;");
    		if (mysqli_num_rows($select_user_devoir) == 1 && mysqli_result($select_user_devoir,0) == $id_user_session){
		  	...
		  	locationhref_admin("?inc=tutorials&do=open_devoir&id_devoir=".$id_devoir);
    	} else locationhref_admin("?inc=tutorials");
		} else locationhref_admin("?inc=tutorials");
  } break;
    ...
    case "open_devoir" : {
    	if (!empty($_POST['send'])){
    		...
    		if (mysqli_num_rows($select_apps) > 0){
    			...
    			redirection(devoir_modifie,"?inc=tutorials&do=open_devoir&id_devoir=".$id_devoir,3,"tips",1);
    		}
    	}
    	else {
    	...
    	if (mysqli_num_rows($select_devoir) == 1){
    		if($devoir = mysqli_fetch_row($select_devoir)){
    			goback_lien("?inc=tutorials&do=open_chapitre&id_chap=".$devoir[1]);
					$titre_devoir = html_ent($devoir[3]);
					$date_publication_devoir = set_date($dateformat,$devoir[5]);
					$date_expiration_devoir = set_date($dateformat,$devoir[6]);
    					...
    					while($devoir_rendu = mysqli_fetch_row($select_devoir_rendu)){
    						...
    						$lien_file = html_ent($devoir_rendu[1]);
    	  					...
    	  					if (mysqli_num_rows($select_note_devoir) == 1)
    							...
    							echo "\n<td class=\"affichage_table\"><a href=\"#\" onClick=\"confirmer('?inc=tutorials&do=delete_devoir_rendu&id_devoir_rendu=".$devoir_rendu[4]."&key=".$key."','".confirm_supprimer_file."')\" title=\"".supprimer."\"><img border=\"0\" src=\"../images/others/delete.png\" width=\"32\" height=\"32\" /></a></td>";
    							echo "</tr>\n";
   						}
   						echo "\n</table>";
...
//*********** Telecharger devoir par classe
    			...
    			echo "</ul>";
				}	else locationhref_admin("?inc=tutorials&do=open_chapitre&id_chap=".$id_chapitre."#devoir");
			} else locationhref_admin("?inc=tutorials&do=open_chapitre&id_chap=".$id_chapitre."#devoir");
		 }
    } break;
  ...
  $select_my_tutos = $connect->query("select * from `" . $tblprefix . "tutoriels` where id_user = $id_user_session;");
	$nbr_trouve = mysqli_num_rows($select_my_tutos);
  if ($nbr_trouve > 0){
		$page_max = ceil($nbr_trouve / $nbr_resultats);
		if ($page <= $page_max && $page > 1 && $page_max > 1)
			$limit = ($page - 1) * $nbr_resultats;
    ...
    $select_my_tutos_limit = $connect->query("select * from `" . $tblprefix . "tutoriels` where id_user = $id_user_session limit $limit, $nbr_resultats;");
    		...
    		echo "<table width=\"100%\" align=\"center\" style=\"border: 1px solid #000000;\"><tr bgcolor=\"#f1d3bd\">\n";
				...
				while($tutoriel = mysqli_fetch_row($select_my_tutos_limit)){
