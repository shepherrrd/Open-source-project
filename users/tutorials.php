...
if (isset($_SESSION['log']) && $_SESSION['log'] == 1 && isset($grade_user_session) && $grade_user_session == "0"){
  ...
  if ($nbr_trouve > 0){
    ...
    case "orderup_devoir" : {
    	if (isset($_GET['key']) && $_GET['key'] == $key){
    		$ce_devoir = mysql_query ("select id_tutoriel, `" . $tblprefix . "devoirs`.id_chapitre, ordre_devoir from `" . $tblprefix . "parties`, `" . $tblprefix . "chapitres`, `" . $tblprefix . "devoirs` where `" . $tblprefix . "chapitres`.id_partie = `" . $tblprefix . "parties`.id_partie and `" . $tblprefix . "devoirs`.id_chapitre = `" . $tblprefix . "chapitres`.id_chapitre and id_devoir = $id_devoir;");
				if (mysql_num_rows($ce_devoir) == 1) {
					$id_tutoriel = mysql_result($ce_devoir,0,0);
					$id_chapitre = mysql_result($ce_devoir,0,1);
					$ordre_devoir = mysql_result($ce_devoir,0,2);
  ...
  case "delete_devoir_rendu" : {
  	if (isset($_GET['key']) && $_GET['key'] == $key){
  		if (isset($_GET['id_devoir_rendu']) && ctype_digit($_GET['id_devoir_rendu']))
    		...
    		$select_user_devoir = mysql_query("select id_user from `" . $tblprefix . "tutoriels`, `" . $tblprefix . "parties`, `" . $tblprefix . "chapitres`, `" . $tblprefix . "devoirs` where `" . $tblprefix . "tutoriels`.id_tutoriel = `" . $tblprefix . "parties`.id_tutoriel and `" . $tblprefix . "parties`.id_partie = `" . $tblprefix . "chapitres`.id_partie and `" . $tblprefix . "chapitres`.id_chapitre = `" . $tblprefix . "devoirs`.id_chapitre and id_devoir = $id_devoir;");
    		if (mysql_num_rows($select_user_devoir) == 1 && mysql_result($select_user_devoir,0) == $id_user_session){
