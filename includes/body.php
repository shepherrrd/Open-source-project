...
if (mysqli_num_rows($select_nombre_caracteres) == 1 && $select_nombre_caracteres->fetch_row() > 0)
	$nombre_caracteres = $select_nombre_caracteres->fetch_row();
...
if (mysqli_num_rows($select_nombre_elements_page) == 1 && $select_nombre_caracteres->fetch_row() > 0)
	$nbr_resultats = $select_nombre_caracteres->fetch_row();
...
if (!empty($_SESSION['log']))
	...
	if (mysqli_num_rows($afficher_profil_aux_visiteurs) == 1) {
		if ($afficher_profil_aux_visiteurs->fetch_row() == 1)
			$afficher_profil = 1;
...
if (isset($_GET['menu']) && ctype_digit($_GET['menu'])){
 ...
 if (mysqli_num_rows($select_statut_menu) == 1) {
  $statut_menu = $select_statut_menu->fetch_row();
  if ($statut_menu == 1) {
	 ...
	 if (mysqli_num_rows($selectmenu) == 1) {
		$type_menu = $selectmenu->fetch_row()[2];
		$lien_menu = $selectmenu->fetch_row()[3];
		switch ($type_menu) {
...
// *** traitement contact ***
	...
	if (mysqli_num_rows($select_statut_contact) == 1) {
		$statut_contact = $select_statut_contact->fetch_row()[1];
		if ($statut_contact == 1) {
			$titre_contact = $select_statut_contact->fetch_row()[0];
...
// *** page d'accueil ***
	...
	if (mysqli_num_rows($selectaccueil) > 0) {
		...
		if (mysqli_num_rows($select_colonnes) > 0 && mysqli_num_rows($selectaccueil) > 1) {
			$multicolonnes = $select_colonnes->fetch_row();
		...
		if ($acces == "*")
			...
			$select_classe = $connect->query("select id_classe from `" . $tblprefix . "apprenants` where id_apprenant = $id_user_session;");
		  if (mysqli_num_rows($select_classe) == 1){
				$id_classe = $select_classer->fetch_row();
				$tab_classes = explode("-",trim($acces,"-"));
				if (in_array($id_classe,$tab_classes))
					$acces_valide = 1;
			}
		...
		if ($acces_valide == 1){
			...
			$selectauteur = $connect->query("select identifiant_user from `" . $tblprefix . "users` where id_user = $id_user;");
			if (mysqli_num_rows($selectauteur) == 1) {
					$auteur = html_ent($selectauteur->fetch_row());
			} else $auteur = inconnu;
