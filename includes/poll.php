	...
	if ($statut_poll == 1) {
	 ...
	 if (mysql_num_rows($select_questions) > 0){
		...
		if (mysql_num_rows($select_questions) == 1){
			...
			if (mysql_num_rows($select_reponses1) > 0){
				if (!empty($_POST['send']) && !empty($_POST['choix_sondage'])) {
					...
					while($reponse1 = mysql_fetch_row($select_reponses1))
 					...
 					if (in_array($choix_sondage, $array_rep1)){
						...
						$select_ip = mysql_query("select * from `" . $tblprefix . "sondage_ip` where ip_vote='$ip_user' and HOUR(heure_vote)=".date('H',time())." and id_question = $id_question1;");
						$select_id = mysql_query("select * from `" . $tblprefix . "sondage_ip` where ip_vote='$user_poll_id' and heure_vote = '00:00:00' and id_question = $id_question1;");						
						if (mysql_num_rows($select_ip) == 0 && mysql_num_rows($select_id) == 0 && !isset($_COOKIE[$cookie_question])) {
							mysql_query("update `" . $tblprefix . "sondage_votes` set nbr_votes=nbr_votes+1 where id_reponse1 = $choix_sondage and id_reponse2 = 0;");
							if (!empty($_SESSION['log']) && !empty($_SESSION['id']))
								mysql_query("INSERT INTO `" . $tblprefix . "sondage_ip` VALUES(NULL,'".$user_poll_id."','00:00:00',$id_question1)");
							else
								mysql_query("INSERT INTO `" . $tblprefix . "sondage_ip` VALUES(NULL,'".$ip_user."',Now(),$id_question1)");
		...
		//*********** Two question *******************
			...
			if (mysql_num_rows($select_reponses1) > 0 && mysql_num_rows($select_reponses2) > 0){
				if (!empty($_POST['send']) && !empty($_POST['choix_sondage1']) && !empty($_POST['choix_sondage2'])) {
					...
					while($reponse1 = mysql_fetch_row($select_reponses1))
 					...
 					if (in_array($choix_sondage1, $array_rep1) && in_array($choix_sondage2, $array_rep2)){
						...
						$ip_user = $_SERVER['REMOTE_ADDR'];
						$select_ip = mysql_query("select * from `" . $tblprefix . "sondage_ip` where ip_vote='$ip_user' and HOUR(heure_vote)=".date('H',time())." and (id_question = $id_question1 or id_question = $id_question2);");
						$select_id = mysql_query("select * from `" . $tblprefix . "sondage_ip` where ip_vote='$user_poll_id' and heure_vote = '00:00:00' and (id_question = $id_question1 or id_question = $id_question2);");
						if (mysql_num_rows($select_ip) == 0 && mysql_num_rows($select_id) == 0 && !isset($_COOKIE[$cookie_question1]) && !isset($_COOKIE[$cookie_question2])) {
							
							mysql_query("update `" . $tblprefix . "sondage_votes` set nbr_votes=nbr_votes+1 where id_reponse1 = $choix_sondage1 and id_reponse2 = $choix_sondage2;");
							
							if (!empty($_SESSION['log']) && !empty($_SESSION['id'])){
								mysql_query("INSERT INTO `" . $tblprefix . "sondage_ip` VALUES(NULL,'".$user_poll_id."','00:00:00',$id_question1)");
								mysql_query("INSERT INTO `" . $tblprefix . "sondage_ip` VALUES(NULL,'".$user_poll_id."','00:00:00',$id_question2)");
							}
							else{
								mysql_query("INSERT INTO `" . $tblprefix . "sondage_ip` VALUES(NULL,'".$ip_user."',Now(),$id_question1)");
								mysql_query("INSERT INTO `" . $tblprefix . "sondage_ip` VALUES(NULL,'".$ip_user."',Now(),$id_question2)");
							}
							locationhref_admin("?poll&vote=2");
						} else locationhref_admin("?poll&vote=1");
 					} else locationhref_admin("?poll&vote=0");
